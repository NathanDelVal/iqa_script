            ...
            for sheet in sheet_reference_name:
                ...
                if sheet == "08-RST_ANL_VRF":
                    ...
                    if lab_col is not None:
                        prepend["Tipo Laboratório"] = ["Interno" if re.search("RMM - LT - Bioagri", str(v), re.IGNORECASE) else "Externo" if not pd.isna(v) else v for v in lab_col]
                    if tipo_col is not None:
                        prepend["Tipo Laboratório"] = ["Interno" if re.search("externo", str(v), re.IGNORECASE) is None else "Externo" if not pd.isna(v) else v for v in tipo_col]
